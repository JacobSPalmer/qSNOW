...
def visualize(
    ...
    chip_pts = [(r, c) for r in range(n_rows) for c in range(n_cols) if r % 2 == c % 2]
    ax.scatter(
    ...
    legend_handles = [
        plt.Line2D(
            ...
            label=f"valid origin (ler ≤ {tau:g})",
        ),
        plt.Line2D(
            [],
            [],
        ...
        plt.Line2D([], [], marker=".", linestyle="", color="0.82", label="chip qubit"),
        Patch(
            facecolor="#2980b9",
            edgecolor="#1b4f72",
            alpha=0.35,
