...
from typing import TYPE_CHECKING, Callable, Dict, List, Optional, Literal, Union
from .models import Qubit, CSSType, Status
...
_NOISE_CHANNEL_OPS = ["CORRELATED_ERROR", "DEPOLARIZE1", "DEPOLARIZE2", "E", "ELSE_CORRELATED_ERROR", "PAULI_CHANNEL_1", "PAULI_CHANNEL_2", "X_ERROR", "Y_ERROR", "Z_ERROR"]
_MULTI_Q_NOISE_CHANNEL_OPS = ["CORRELATED_ERROR","DEPOLARIZE2","PAULI_CHANNEL_2"]
_TWO_CLIFFORD_OPS = ["CNOT", "CX", "CY", "CZ", "ISWAP", "ISWAP_DAG", "SQRT_XX", "SQRT_XX_DAG", "SQRT_YY", "SQRT_YY_DAG", "SQRT_ZZ", "SQRT_ZZ_DAG", "SWAP", "XCX", "XCY", "XCZ", "YCX", "YCY", "YCZ", "ZCX", "ZCY", "ZCZ"]
_SINGLE_CLIFFORD_OPS = ["C_XYZ", "C_ZYX", "H", "H_XY", "H_XZ", "H_YZ", "S", "SQRT_X", "SQRT_X_DAG", "SQRT_Y", "SQRT_Y_DAG", "SQRT_Z", "SQRT_Z_DAG", "S_DAG"]
_PAULI_OPS = ["I","X","Y","Z"]
_MEASURE_OPS = ["M", "MPP", "MR", "MRX", "MRY", "MRZ", "MX", "MY", "MZ"]
_RESETS_OPS = ["R", "RX", "RY", "RZ", "MR", "MRX", "MRY", "MRZ"]
_ANNOTATION_OPS = ["DETECTOR", "OBSERVABLE_INCLUDE", "QUBIT_COORDS", "SHIFT_COORDS", "TICK", "REPEAT"]
...
def _any_qubits_trigger(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> bool:
    return True

def _all_qubits_trigger(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> bool:
...
def _all_measures_trigger(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> bool:
...
def _x_measures_trigger(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> bool:
    b = all(all(qubits.get(t, Qubit()).is_x_measure() for t in i) for i in targ_indexes)
    # print(f'triggered on all_measures -> {b}')
    return b

def _z_measures_trigger(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> bool:
    return all(all(qubits.get(t, Qubit()).is_z_measure() for t in i) for i in targ_indexes)

# -----------------------------------------------------------------------
# Rule filters -> What the qubits specific channels should be applied to
# -----------------------------------------------------------------------

def _blank_filter(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
    return []

def _all_qubits_filter(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
    return [[k] for k in qubits]

def _active_filter(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
    return targ_indexes

def _idle_filter(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
    ...
    return [[k] for k in qubits if k not in [item for sublist in targ_indexes for item in sublist]]

def _all_measures_filter(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
    return [[k] for k,v in qubits.items() if v.is_measure()]

def _data_filter(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
    return [[k] for k,v in qubits.items() if v.is_data()]

def _x_measures_filter(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
    return [[k] for k,v in qubits.items() if v.is_x_measure()]

def _z_measures_filter(targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
...
type FilterFunc = Callable[[List[List[int]], Dict[int, Qubit]], List[List[int]]] #specifies what
...
@dataclass
class ChannelRule():
    channel: Literal["CORRELATED_ERROR", "DEPOLARIZE1", "DEPOLARIZE2", "E", "ELSE_CORRELATED_ERROR", "PAULI_CHANNEL_1", "PAULI_CHANNEL_2", "X_ERROR", "Y_ERROR", "Z_ERROR"]
    filter: Union[Literal['active', 'idle', 'all_qubits', 'all_measures', 'data', 'x_measures','z_measures'], str] #This filter determines what qubits this channel will be applied to
    scalar: float = 1.0
...
class InjectionRule():
    operation: str
    trigger: Union[Literal['any', 'all_qubits', 'all_measures', 'data', 'x_measures','z_measures'], str] #This filter determines IF the before or after channels will be applied to this operation
...
_DEFAULT_FILTERS:   List[Filter]  = [Filter('active', _active_filter), Filter('idle',_idle_filter), Filter('all_qubits',_all_qubits_filter), Filter('all_measures',_all_measures_filter), Filter('data',_data_filter), Filter('x_measures',_x_measures_filter), Filter('z_measures', _z_measures_filter)]
_DEFAULT_TRIGGERS:  List[Trigger] = [Trigger('any', _any_qubits_trigger), Trigger('all_qubits', _all_qubits_trigger), Trigger('all_measures', _all_measures_trigger), Trigger('data',_data_trigger), Trigger('x_measures',_x_measures_trigger),Trigger('z_measures',_z_measures_trigger)]
...
class Ruleset():
    ...
    def check_trigger(self, name: str, targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> bool:
    ...
    def apply_filter(self, name: str, targ_indexes: List[List[int]], qubits: Dict[int, Qubit]) -> List[List[int]]:
    ...
    def add_rule(self, injection_rule: InjectionRule, priority_index: Optional[int] = None) -> None:
